...
/**
 * Verifies that the step-up authentication policy of the endpoints annotated with the
 * {@code @AuthenticationContext} annotation is enforced for the tests authenticated with
 * the {@code @TestSecurity} annotation, which bypasses the OIDC token verification that
 * normally enforces the policy. The required 'acr' and 'auth_time' claim values are
 * defined with the {@code @OidcSecurity} annotation.
 */
